...
# The optimal distance for pen contact (adjust based on your pen/paper)
PEN_CONTACT_Y = ORIGIN_Y + 2  # Slightly forward for gentle contact
PEN_DRAWING_Y = ORIGIN_Y + 4  # Drawing pressure (more forward = more pressure)
PEN_RETRACT_Y = ORIGIN_Y - 30  # Safe retraction distance
...
class VerticalDrawingRobot:
    ...
    def gentle_pen_down(self, x, z):
        """Gently lower the pen to the drawing surface."""
        if self.pen_is_down:
            return
            
        # First move to position above the paper
        self.mc.send_coords([x, PEN_RETRACT_Y, z] + self.DRAWING_ORIENTATION, TRAVEL_SPEED, 0)
        time.sleep(0.5)
        
        # Approach slowly to contact position
        self.smooth_approach([x, PEN_CONTACT_Y, z] + self.DRAWING_ORIENTATION, APPROACH_SPEED)
        time.sleep(0.2)
        
        # Apply gentle drawing pressure
        self.mc.send_coords([x, PEN_DRAWING_Y, z] + self.DRAWING_ORIENTATION, APPROACH_SPEED // 2, 0)
        time.sleep(0.2)
    ...
    def gentle_pen_up(self):
        ...
        if current:
            ...
            self.mc.send_coords([current[0], PEN_CONTACT_Y, current[2]] + self.DRAWING_ORIENTATION, LIFT_SPEED, 0)
            time.sleep(0.1)
            
            # Then retract
            self.mc.send_coords([current[0], PEN_RETRACT_Y, current[2]] + self.DRAWING_ORIENTATION, LIFT_SPEED, 0)
            time.sleep(0.3)
    ...
    def draw_line_segment(self, from_point, to_point):
        ...
        if distance < MIN_SEGMENT_LENGTH:
            ...
            self.mc.send_coords([x2, PEN_DRAWING_Y, z2] + self.DRAWING_ORIENTATION, DRAWING_SPEED, 0)
        else:
            ...
            for i in range(1, num_points + 1):
                ...
                x = x1 + (x2 - x1) * ratio
                z = z1 + (z2 - z1) * ratio
                self.mc.send_coords([x, PEN_DRAWING_Y, z] + self.DRAWING_ORIENTATION, DRAWING_SPEED, 0)
                time.sleep(0.05)
    ...
    def test_drawing_area(self):
        ...
        margin = 20
        test_corners = [
            (ORIGIN_X + margin, ORIGIN_Z - margin),
            (ORIGIN_X + DRAWING_AREA_WIDTH_MM - margin, ORIGIN_Z - margin),
            (ORIGIN_X + DRAWING_AREA_WIDTH_MM - margin, ORIGIN_Z - DRAWING_AREA_HEIGHT_MM + margin),
            (ORIGIN_X + margin, ORIGIN_Z - DRAWING_AREA_HEIGHT_MM + margin),
            (ORIGIN_X + margin, ORIGIN_Z - margin),  # Close the rectangle
        ]
        
        # Move to start position
        print("Moving to start position...")
        start_x, start_z = test_corners[0]
        self.mc.send_coords([start_x, PEN_RETRACT_Y, start_z] + self.DRAWING_ORIENTATION, TRAVEL_SPEED, 0)
        time.sleep(2)
        
        # Draw test rectangle
        print("Drawing test rectangle...")
        self.gentle_pen_down(start_x, start_z)
    ...
    def draw_sketch(self, sketch_image):
        ...
        for i, contour in enumerate(contours):
            ...
            self.gentle_pen_down(start_x_mm, start_z_mm)
            
            # Draw the contour with smooth segments
            prev_point = (start_x_mm, start_z_mm)
            for j in range(1, len(contour)):
                ...
                self.draw_line_segment(prev_point, (x_mm, z_mm))
                prev_point = (x_mm, z_mm)
